...
struct ProviderDetailPanel: View {
    ...
    private var isMuse: Bool {
        descriptor.providerKey.caseInsensitiveCompare("Muse") == .orderedSame
    }
    ...
    private var museLoginSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack(spacing: 12) {
                providerDetailIcon(isConfigured ? "checkmark.seal.fill" : "person.crop.circle.badge.checkmark")

                VStack(alignment: .leading, spacing: 3) {
                    Text(isConfigured ? "Muse login available" : "Muse login unavailable")
                        .font(.system(size: 13, weight: .semibold))
                        .foregroundStyle(.primary)

                    Text("Sign in once with `muse login` to use your existing Muse account.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
            ...
            Text("Muse Code does not need to stay running. The first use may ask for Keychain access.")
                .font(.caption)
                .foregroundStyle(.secondary)
                .fixedSize(horizontal: false, vertical: true)
