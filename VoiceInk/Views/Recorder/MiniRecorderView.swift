        ...
        HStack(spacing: 0) {
            HStack(spacing: 6) {
                ...
                if shouldShowCancelButton {
                    RecorderCancelButton(action: onCancelTapped, helpLabel: "Cancel recording", accessibilityLabel: "Cancel recording")
                        .transition(.opacity)
                }
            }
            .padding(.leading, 10)
            ...
            Spacer(minLength: 0)
            RecorderStatusDisplay(currentState: stateProvider.recordingState, audioMeterProvider: recorder.audioMeterSnapshot)
            Spacer(minLength: 0)
            RecorderModeButton(buttonSize: 22, padding: EdgeInsets()).padding(.trailing, 12)
        }
