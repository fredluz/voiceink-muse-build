...
struct NotchRecorderView<S: RecorderStateProvider & ObservableObject>: View {
    ...
    // Cancel discards the active recording/transcription without delivery.
    ...
    private var displayState: DisplayState {
        if assistantSession.isVisible { return .assistant }
    ...
    private var sideExpansion: CGFloat {
        switch displayState {
        case .liveText: return transcriptSideExpansion
        case .assistant: return assistantSideExpansion
        case .active, .collapsed: return recordingSideExpansion
        }
    }
    ...
    private var shouldShowCancelButton: Bool {
        switch stateProvider.recordingState {
        case .starting, .recording, .transcribing, .enhancing: return true
        case .idle, .busy: return false
        }
    }

    private var liveAssistantFollowUpText: String {
        stateProvider.recordingState == .recording ? stateProvider.partialTranscript : ""
    }
        ...
        ZStack {
            ...
            HStack(spacing: 14) {
                if shouldShowCloseButton {
                    ...
                    RecorderRecordButton(recordingState: stateProvider.recordingState, action: onRecordButtonTapped)
                }
                if shouldShowCancelButton {
                    RecorderCancelButton(action: onCancelTapped).transition(.opacity)
                }
            ...
            .animation(displayState != .collapsed ? expandAnimation.delay(0.09) : collapseAnimation, value: displayState)

            HStack(spacing: 0) {
                Spacer(minLength: 0)
                RecorderStatusDisplay(currentState: stateProvider.recordingState, audioMeter: recorder.audioMeter, menuBarHeight: notchHeight)
            }
    ...
    private var liveTextPanel: some View {
        VStack(spacing: 0) {
            if displayState == .liveText {
                ...
                LiveTranscriptView(text: stateProvider.partialTranscript).padding(.horizontal, 8)
            }
        }
    ...
    private var assistantPanel: some View {
        VStack(spacing: 0) {
            if displayState == .assistant {
                Divider().background(Color.white.opacity(0.15))
                AssistantPanelView(session: assistantSession, liveFollowUpText: liveAssistantFollowUpText, onSend: onAssistantFollowUp)
            }
        }
