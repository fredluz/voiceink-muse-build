...
struct OnboardingTranscriptionSetupCard: View {
    ...
    private var isMuse: Bool {
        selectedProvider?.modelProvider == .muse
    }
    ...
    private var canVerify: Bool {
        (isMuse || !trimmedAPIKey.isEmpty) && !isVerifying
    }
    ...
    private var cloudSetup: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            if isMuse {
                Text("Sign in once with `muse login`. VoiceInk reuses that login; Muse Code does not need to stay running.")
                    .font(.caption)
                    .foregroundColor(AppTheme.Text.secondary)
                verificationFooter
            } else if isSelectedProviderConnected {
        ...
        HStack(alignment: .center, spacing: 12) {
            ...
            Button(action: verifyAPIKey) {
                HStack(spacing: 6) {
                    ...
                    Text(isMuse
                        ? (isVerifying ? LocalizedStringKey("Checking Muse login") : LocalizedStringKey("Use Muse login"))
                        : (isVerifying ? LocalizedStringKey("Testing...") : LocalizedStringKey("Test connection")))
                }
                .font(.system(size: 12, weight: .semibold))
    ...
    private var statusLine: some View {
        if let verificationMessage {
            ...
            Text(isMuse ? "Use your saved Muse login to continue." : "Test the connection to continue.")
                .font(.system(size: 12))
                .foregroundColor(AppTheme.Text.secondary)
        }
    }
    ...
    private func refreshVerificationState() {
        ...
        verificationMessage =
            ...
            ? selectedProvider.map {
                isMuse
                    ? String(localized: "Muse login available")
    ...
    private func useMuseLogin() {
        guard let providerKey = selectedProvider?.providerKey, !isVerifying else { return }
        isVerifying = true
        verificationSucceeded = false
        verificationMessage = nil
        verificationDetailMessage = nil
        ...
        Task { @MainActor in
            do {
                ...
                guard selectedProvider?.providerKey == providerKey else { return }
                verificationSucceeded = true
                verificationMessage = String(localized: "Muse login available")
            } catch {
                guard selectedProvider?.providerKey == providerKey else { return }
                verificationMessage = String(localized: "Could not use Muse login")
                verificationDetailMessage = error.localizedDescription
            }
            isVerifying = false
            onVerificationChanged()
        }
