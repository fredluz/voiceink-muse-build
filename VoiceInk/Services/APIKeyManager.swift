...
final class APIKeyManager {
    ...
    private static let museProviderKey = "muse"

    private static func isMuseProvider(_ provider: String) -> Bool {
        provider.lowercased() == museProviderKey
    }
    ...
    func saveAPIKey(_ key: String, forProvider provider: String) -> Bool {
        guard !Self.isMuseProvider(provider) else {
            return false
        }
    ...
    func hasAPIKey(forProvider provider: String) -> Bool {
        if Self.isMuseProvider(provider) {
            return MuseLoginCredentials.isAvailable
        }
        let keyIdentifier = keychainIdentifier(forProvider: provider)
