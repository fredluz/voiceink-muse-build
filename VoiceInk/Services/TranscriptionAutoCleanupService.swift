...
class TranscriptionAutoCleanupService {
    ...
    private let minimumTranscriptionLifetime: TimeInterval = 6 * 60 * 60
    ...
    @objc private func handleTranscriptionCompleted(_ notification: Notification) {
        ...
        guard Date().timeIntervalSince(transcription.timestamp) >= minimumTranscriptionLifetime else {
            return
        }
    ...
    private func sweepOldTranscriptions(modelContext: ModelContext) async {
        ...
        let effectiveRetention = max(
            TimeInterval(effectiveMinutes * 60),
            minimumTranscriptionLifetime
        )
        let cutoffDate = Date().addingTimeInterval(-effectiveRetention)
    ...
    private func cleanupOrphanAudioFiles(modelContext: ModelContext) async {
        ...
        do {
            ...
            for fileURL in filesInDirectory {
                ...
                guard let fileDate,
                    Date().timeIntervalSince(fileDate) >= minimumTranscriptionLifetime
                else { continue }
