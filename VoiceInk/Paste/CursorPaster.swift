...
class CursorPaster {
    ...
    static func startPasteAtCursor(
        _ text: String,
        destinationSnapshot: PasteDestinationSnapshot? = nil
    ) -> Task<PasteResult, Never> {
        let snapshot = destinationSnapshot ?? PasteDestinationSnapshot.capture()
        return Task { @MainActor in
            await performPasteSession(text, destinationSnapshot: snapshot)
        }
    }
    ...
    static func pasteAtCursorAndWaitUntilPosted(
        ...
        await startPasteAtCursor(text, destinationSnapshot: destinationSnapshot).value
    }
