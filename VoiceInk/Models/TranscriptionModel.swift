...
struct CustomCloudModel: TranscriptionModel, Codable {
    ...
    init(from decoder: Decoder) throws {
        ...
        // Tolerate legacy/imported records without the derived language map.
        // Keep the persisted identity and endpoint usable in that case.
        supportedLanguages =
            try container.decodeIfPresent([String: String].self, forKey: .supportedLanguages)
            ?? LanguageDictionary.forProvider(isMultilingual: isMultilingualModel)
